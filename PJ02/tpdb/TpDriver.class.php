...
abstract class TpDriver {
     ...
     * @param boolean $getDriver  不执行只是返回Driver用于迭代查询
     * @return mixed
     */
    public function query($str,$fetchSql=false,$getDriver=false) {
    ...
    public function getIterator() {
        //返回数据集
        $this->numRows = 0;
        while ($result = $this->PDOStatement->fetch(\PDO::FETCH_ASSOC)) {
            $this->numRows++;
            yield $result;
        }
    }
    ...
    public function select($options=array()) {
        ...
        $this->parseBind(!empty($options['bind'])?$options['bind']:array());
        $sql    = $this->buildSelectSql($options);
        $result   = $this->query($sql,!empty($options['fetch_sql']) ? true : false, !empty($options['get_driver']) ? true : false);
        return $result;
    }
