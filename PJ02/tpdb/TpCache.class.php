...
class TpCache {
    ...
    static private $instance = [];   //  当前数据库连接实例
    ...
    public function connect($type='',$options=array()) {
        ...
        $class  =   strpos($type,'\\')? $type : 'TpCache'.ucwords(strtolower($type));
    ...
    static function getInstance($type='',$options=array()) {
		...
		if(!isset(self::$instance[$guid])){
			$obj = new TpCache();
            self::$instance[$guid] = $obj->connect($type,$options);
			unset($obj);
		}
     ...
     * @param array $config
     */
    static public function unsetInstance($type='',$options=array())
    {
        $guid = $type.to_guid_string($options);
        if (isset(self::$instance[$guid])) {
            self::$instance[$guid]->close();
            self::$instance[$guid] = null;
            unset(self::$instance[$guid]);
        }
    }
