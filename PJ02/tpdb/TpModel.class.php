...
class TpModel {
    ...
    public function select($options=array()) {
        ...
        // 返回$this->db数库启动器,方便调用$driver->getIterator ($driver==$this->db)
        if (!empty($options['get_driver'])) {
            return $resultSet;
        }
    ...
    public function db($linkNum='',$config='',$force=false) {
        ...
        if(!isset($this->_db[$linkNum]) || $force ) {
            ...
            $this->_db[$linkNum]            =    TpDb::getInstance($config);
        }elseif(NULL === $config){
    ...
    public function where($where,$parse=null){
        ...
        if(is_string($where) && '' != $where){
            $map    =   array();
            /*modify by cwhao 字符串where多次调用 2016-08-10*/
            //$map['_string']   =   $where;
